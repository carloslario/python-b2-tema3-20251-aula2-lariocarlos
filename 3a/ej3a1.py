...
class Product(ABC):
    ...
    def price(self, value: float) -> None:
        ...
        if value <= 0:
            raise ValueError (f"{value}: el valor introducido es negativo.")
        self._price = value
...
if __name__ == "__main__":
    ...
    book_c = Book("Conde de Monte Cristo",20, "Alexandre Dumas", "9876-543-210")
    book_c.describe_product()
    print(f"Accediendo al precio a través del getter: {book_c.price} €.")
    value = float(input("Introduce un nuevo valor para el producto: "))
    book_c.price = value
    print(f"El nuevo precio del producto es: {book_c.price} €.")
